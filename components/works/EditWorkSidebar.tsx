...
interface EditWorkSidebarProps {
  work: WorkItem | null;
  onClose: () => void;
  onSave: (id: number, title: string, tags: string[], thumbnails: string[]) => void;
}

// ─── Component ────────────────────────────────────────────────────────────────

export default function EditWorkSidebar({ work, onClose, onSave }: EditWorkSidebarProps) {
  const isOpen = !!work;
  ...
  // ── Populate when work changes ─────────────────────────────────────────────

  useEffect(() => {
    if (work) {
      setTitle(work.title);
      setTags([...work.tags]);
      // Re-use existing thumbnails as previews (no type info needed — show as image)
      setPreviews((work.thumbnails || []).map((url) => ({ url, isVideo: false })));
    }
  }, [work]);
  ...
  function handleSubmit() {
    ...
    onSave(work.id, title.trim(), tags, previews.map((p) => p.url));
    onClose();
  }

  const canSubmit = title.trim().length > 0 && tags.length > 0;
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside
        className={`fixed top-0 right-0 h-full w-[400px] bg-[#FDFDFD] shadow-2xl z-50 flex flex-col transform transition-transform duration-300 ease-out ${
      ...
      >
        ...
        <div className="bg-[#002B66] px-5 py-4 flex items-start justify-between shrink-0">
          <div>
            <h2 className="text-[16px] font-bold text-white leading-snug">Edit Proof of Work</h2>
            <p className="text-[12px] text-[#95979C] mt-0.5">Update your work details and media.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-6 bg-[#FBFBFB]">
          ...
          {/* ── Photos & Videos ── */}
          <div>
            <label className="text-[13px] font-semibold text-[#1A1A1D] mb-2 block">Photos &amp; Videos</label>
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-[13px] text-[#5A5C60] bg-[#FDFDFD] border border-[#DCDCDC] rounded-lg px-3 py-2.5 outline-none focus:border-[#66A6FF] focus:ring-1 focus:ring-[#B2D3FF] transition-colors"
            />
          </div>
          ...
          <div>
            ...
            <SkillsInput tags={tags} onChange={setTags} chipVariant="blue-bordered" />
          </div>
